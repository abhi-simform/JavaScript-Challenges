...
console.log("=== All Users (Map Method) ===");
users.map((user) => console.log(user.displayName));

// Using filter method create an array of users who are senior engineers
console.log("=== Senior Users (Filter Method) ===");
const srUsers = users
  .filter(
    (user) => user.jobtitle.includes("Sr") && console.log(user.displayName)
  )
  .map((srUser) => console.log(srUser.employeeNumber));

// Using reduce method create the sum of employeeNumber of all users
console.log("=== Sum of employeeNumber (Reduce Method) ===");

const sumEmpNumber = users.reduce((acc, user) => acc + user.employeeNumber, 0);
console.log(sumEmpNumber);
